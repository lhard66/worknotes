...
### 6.5 Saturday
### 6.7 Monday
1. 绘图项目使用@df/eslint-config-standard。版本同步问题，应于现有项目使用版本一致！完成！
2. 实现各应用之间组件的调用。
   1. 实现已打开的子应用窗体，可以被主应用调用。完成！
   2. 调用成功后，处理样式渲染问题。完成！
   3. 子应用未打开时，主应用也可调用子应用的窗体。完成！
   4. 支持子应用A调用子应用B的窗体。完成！
   5. 提取出公共的依赖库。暂不需支持，因为公共资源提取后，子应用无法独立运行。
